 ...
 */
export const generateNarrationAudio = async (text: string, sceneId: string): Promise<string> => {
  if (!elevenLabsClient) {
  ...
  try {
    ...
    throw new AppError(
      `Failed to generate TTS audio: ${error instanceof Error ? error.message : String(error)}`,
      500,
      false
    );
  }
...
export const mergeVideoWithAudio = async (
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      (ffmpeg as any).ffprobe(videoPath, (videoErr: Error | null, videoMetadata: any) => {
        if (videoErr) {
        ...
        // Get audio duration
        (ffmpeg as any).ffprobe(audioPath, (audioErr: Error | null, audioMetadata: any) => {
          if (audioErr) {
    ...
    throw new AppError(
      `Failed to merge video with audio: ${error instanceof Error ? error.message : String(error)}`,
      500,
      false
    );
  }
