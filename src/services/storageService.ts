 ...
 */
export const uploadFile = async (localFilePath: string, destinationFileName: string): Promise<string> => {
  try {
    if (!storage || !bucket) {
 ...
 */
export const uploadVideo = async (localFilePath: string, sceneId: string): Promise<string> => {
 ...
 */
export const uploadAudio = async (localFilePath: string, sceneId: string): Promise<string> => {
  // Generate a unique filename based on the scene ID
  const fileExtension = path.extname(localFilePath);
  const timestamp = Date.now();
  const hash = createHash('md5').update(`${sceneId}-${timestamp}`).digest('hex').substring(0, 8);
