...
// Define a type for the valid log level strings
type LogLevelString = keyof typeof levels;

// Determine the current log level's severity from config, default to 'info'
const currentLogLevelName = (config.logging.level || 'info').toLowerCase() as LogLevelString;
const currentLogLevelSeverity: number = levels[currentLogLevelName] !== undefined
 ...
 */
const log = (level: LogLevelString, message: string, ...optionalParams: any[]): void => {
